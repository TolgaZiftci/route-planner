...
public class RouteService {
     ...
     */
    public List<FullRoute> getAllRoutes(int originLocation, int destLocation) {
        RouteFinder finder = new RouteFinder(originLocation, destLocation);
        finder.init(getPathGraph(originLocation, destLocation));
        return finder.findRoutes();
    }

    private List<TransportationSummary> getPathGraph(int originLocation, int destLocation) {
        ...
        List<TransportationSummary> secondPaths = firstPaths.stream().map(initialLocation ->
                transportationRepository.findKeysByOriginLocation_Id(initialLocation.destId())).flatMap(List::stream).toList();
        ...
        List<TransportationSummary> thirdPaths = secondPaths.stream().map(initialLocation ->
                transportationRepository.findKeysByOriginLocation_IdAndDestLocation_Id(initialLocation.destId(), destLocation)).flatMap(List::stream).toList();

        return Stream.of(firstPaths, secondPaths, thirdPaths).flatMap(List::stream).toList();
    }
}
